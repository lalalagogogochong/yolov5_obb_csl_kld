...
from utils.loss import ComputeLoss, ComputeLoss_kld
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, loss_fun = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze, opt.loss_fun
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), loss_fun=opt.loss_fun).to(device)  # create
    ...
    if loss_fun == 'CSL':
        ...
        LOGGER.info('Please confirm the form of loss function: CSL or KLD')
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = val.run(data_dict,
                                            ...
                                            callbacks=callbacks,
                                            compute_loss=compute_loss,
                                            loss_fun=loss_fun)  # val best model with plots
                    if is_coco:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--loss_fun', type=str, default='KLD',help='CSL or KLD')
